...
def analyze(path: str) -> Analysis:
    ...
    """
    con = duckdb.connect()
    (total, duplicates, below_baseline_0500) = con.execute(
        _PARSED_CTE
        + """
        SELECT
            ...
            (SELECT COUNT(*) FROM parsed p
             WHERE CAST(SUBSTR(p.baseline, 2, 4) AS INTEGER) < 500
             AND NOT EXISTS (
                 ...
                 AND CAST(SUBSTR(q.baseline, 2, 4) AS INTEGER) >= 500
             ))
        """,
        [path],
    ).fetchone()
    ...
    return Analysis(
        ...
        below_baseline_0500=below_baseline_0500,
